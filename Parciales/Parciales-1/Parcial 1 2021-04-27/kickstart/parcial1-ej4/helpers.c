...
static unsigned int process_FILE(FILE *file, song_t playlist[], unsigned int max_size) {
    unsigned int i = 0u;
    while (!feof(file)) {
        // Chequeo si el i, que corresponde al size del array, no supera el maximo permitido.
        if (i > max_size) {
            fprintf(stderr, "Array is too long! Max playlist size reached: %u \n", max_size);
            exit(EXIT_FAILURE);
        }else{
            ...
            // Chequeo si se leyo correctamente, fscanf devuelve la cantidad de elementos leidos correctamente
            if (readed != 2)
            {
                fprintf(stderr, "Invalid array. Error reading song data\n");
                exit(EXIT_FAILURE);
            }
            ++i;
        }
